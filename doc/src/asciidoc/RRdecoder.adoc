...
To select  selectthe Control key while you click it will add (or subtract) that single entry.
Control-clicking an entry will reverse the selection state of that entry.
ion to multiple entries, hold down the shift key
while you clielection to multiple entries, hold down the shift key
...
Manufacturer is the value that was entered when the decoder was first entered into 
...
step for each locoomotive when you profile them, you will likely see a wide
variation in rows of the display. The window will show the throttle percentage (as
a decimal fraction), the speed step (from 1 to 126) and the recorded forward and
...
* View Key Value Pairs will show a list of all the keys assinged to the selected 
engines and the value of each. The keys reflect information that DecoderPro 
stores about each engine. Examples of this information include the Roster Group
for the engine (if you are using that capability), and, if you are using 
...
For non-indexed CVs simply enter the CV number (from 0 to 255). Indexed CVs are
